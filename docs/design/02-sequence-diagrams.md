...
sequenceDiagram
    ...
    상품 목록 조회 API->>+브랜드: 브랜드 존재 여부 확인
    
    alt 브랜드가 존재하지 않을 경우
        브랜드-->>상품 목록 조회 API: 오류 반환
        상품 목록 조회 API-->>사용자: "상품 목록을 조회할 수 없습니다."
    else 브랜드가 존재하거나 빈 값인 경우
        브랜드-->>-상품 목록 조회 API: 브랜드 ID 반환
...
- 사용자는 **각 상품에 한 번만 좋아요** 를 누를 수 있다.
- 상품에 대한 **좋아요 추가/해제** 기능은 멱등하게 동작한다.
```mermaid
sequenceDiagram
actor 사용자
participant 상품 좋아요 등록/취소 API
participant 회원
participant 상품
participant 좋아요

    사용자->>+상품 좋아요 등록/취소 API: 좋아요 등록/취소 요청 (상품 ID)

    상품 좋아요 등록/취소 API->>+회원: 사용자 존재 여부 확인
    alt 사용자가 존재하지 않을 경우
        회원-->>상품 좋아요 등록/취소 API: 오류 반환
        상품 좋아요 등록/취소 API-->>사용자: "사용자가 존재하지 않습니다."
    else 사용자가 존재할 경우
        회원-->>-상품 좋아요 등록/취소 API: 사용자 ID 반환
        상품 좋아요 등록/취소 API->>+상품: 상품 존재 여부 확인
        alt 상품이 존재하지 않을 경우
            상품-->>상품 좋아요 등록/취소 API: 오류 반환
            상품 좋아요 등록/취소 API-->>사용자: "해당 상품이 존재하지 않습니다."
        else 상품이 존재할 경우
            상품-->>-상품 좋아요 등록/취소 API: 상품 ID 반환
            상품 좋아요 등록/취소 API->>+좋아요: 좋아요 등록 또는 취소 요청 (상품 ID, 사용자 ID)
            좋아요->>좋아요: 좋아요 등록 또는 취소
            좋아요-->>-상품 좋아요 등록/취소 API: 좋아요 등록 또는 취소 응답
            상품 좋아요 등록/취소 API-->>-사용자: 좋아요 등록 또는 취소 완료
        end
    end
```
...
participant 좋아요
    ...
    좋아요 목록 조회 API->>+회원: 사용자 존재 여부 확인
    alt 사용자가 존재하지 않을 경우
        회원-->>좋아요 목록 조회 API: 오류 반환
        좋아요 목록 조회 API-->>사용자: "사용자가 존재하지 않습니다."
    else 사용자 존재할 경우
        회원-->>-좋아요 목록 조회 API: 사용자 ID 반환
...
sequenceDiagram
    ...
    주문 생성 API->>+회원: 사용자 존재 여부 확인
    alt 사용자가 존재하지 않을 경우
        회원-->>주문 생성 API: 오류 반환
        주문 생성 API-->>사용자: "사용자가 존재하지 않습니다."
    else 사용자 존재할 경우
        회원-->>-주문 생성 API: 사용자 ID 반환
...
participant 주문
    ...
    주문 목록 조회 API->>+회원: 사용자 존재 여부 확인
    alt 사용자가 존재하지 않을 경우
        회원-->>주문 목록 조회 API: 오류 반환
        주문 목록 조회 API-->>사용자: "사용자가 존재하지 않습니다."
    else 사용자 존재할 경우
        회원-->>-주문 목록 조회 API: 사용자 ID 반환
...
participant 주문
    ...
    단일 주문 상세 조회 API->>+회원: 사용자 존재 여부 확인
    alt 사용자가 존재하지 않을 경우
        회원-->>단일 주문 상세 조회 API: 오류 반환
        단일 주문 상세 조회 API-->>사용자: "사용자가 존재하지 않습니다."
    else 사용자 존재할 경우
        회원-->>-단일 주문 상세 조회 API: 사용자 ID 반환
